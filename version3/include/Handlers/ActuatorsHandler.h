...
void configuration_actuators_handler()
{
	...
	// Питание драйверов - включены
	actuator_motor_list.gearbox.set_enable(HIGH);
	actuator_motor_list.clutch.set_enable(HIGH);
	actuator_motor_list.rudder.set_enable(HIGH);
	actuator_motor_list.brake.set_enable(HIGH);

	// Доп-настройки драйверов
	actuator_motor_list.brake.set_reverse(true);

	// Связать поршень с мотором.
	actuator_piston_list.gearbox.configuration(actuator_motor_list.gearbox, 0, 0, (long) 500000); // Коробка передач
	actuator_piston_list.clutch .configuration(actuator_motor_list.clutch,  0, 0, (long) 500000); // Сцепление
	actuator_piston_list.rudder .configuration(actuator_motor_list.rudder,  0, 0, (long) 500000); // Руль
	actuator_piston_list.brake  .configuration(actuator_motor_list.brake,   0, 0, (long) 500000); // Тормоз
